...
- [x] Settings — update call windows, timezone, and email automation preferences from the web
...
## Product Evolution
- [x] On-demand calls — user can request an accountability call anytime via WhatsApp ("call me now" schedules an immediate callback)
...
- [x] Auto-task from emails — implemented with Gmail scan, per-user opt-in, confidence scoring, automated/no-reply sender filtering, thread dedupe, and `TaskService` creation
